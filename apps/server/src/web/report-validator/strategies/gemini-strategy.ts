...
export class GeminiReportValidatorStrategy implements IReportValidatorStrategy {
    async checkReportErrors(report: string, findings: string, prompt: string, userId: string, threadId?: string) {
        ...
        try {
            // 1) Pull dynamic config override for Gemini Validator
            const aiConfig = aiConfigService.getValidatorConfig("gemini");
            ...
            const responseText = await LLMWrapper.generateChatCompletion({
                ...
                topP: aiConfig.TOP_P,
                providerName: "ReportValidator:Gemini",
            ...
            logger.error("Gemini Validator Strategy API error:", error);
            throw error;
        }
    }
}