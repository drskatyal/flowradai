...
export const Dialog = ({
  ...
  const dialogContent = (
    <DialogContent
      onInteractOutside={(e) => {
        if (!allowInteractionOutside && !closeOnOutsideClick) e.preventDefault(); // Block outside click from closing the dialog unless explicitly allowed
      }}
      ...
      {(headerTitle || headerDescription) && (
        <DialogHeader className={classNames?.header}>
          ...
          {draggable && headerTitle && (
            <div
              className={`dialog-drag-handle cursor-move px-4 py-2 bg-muted text-muted-foreground font-medium rounded-t-md ${classNames?.headerTitle ?? ""
                }`}
            >
              {Icon && <Icon className="w-5 h-5 text-neutral-800 dark:text-neutral-200" strokeWidth={2} />}
              {headerTitle}
            </div>
          )}
          {!draggable && headerTitle && (
            <DialogTitle className={classNames?.headerTitle}>
              {Icon && <Icon className="w-5 h-5 text-neutral-800 dark:text-neutral-200" />}
              {headerTitle}
            </DialogTitle>
          )}
          {headerDescription && (
